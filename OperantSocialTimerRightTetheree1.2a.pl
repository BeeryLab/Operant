#!/usr/bin/env perl
#Annaliese Beery, July 2017, updated 2019
...
sub drawTitle()
{
	...
	print "          ` ^^   ^^              2017-2019 Behavioral Scoring Program\n";
	print "\n";
	print "*******************************************************************\n\n";
}
...
#	print"   huddling    not huddling       (1/2 or more)      not huddling    huddling\n\n";
	...
	print"               Left (start)          Tube           Right           Right\n";
	print"               Operant box       (1/2 or more)      not huddling    huddling\n";
...
#	$left_hud = $durations->[0]/60;
	...
	$solo = $durations->[2]/60;  #this is tube time
	...
	if ($last_state == 4) {         #4=ended huddling
		$soc_chamb_increment = 1;	#If end w L huddling, this avoids off by one error in chamber entry count
		}
	$soc_chamb_entries = ($iterations->[3])-($iterations->[4]) + $soc_chamb_increment;
...
#print raw time values	
	...
	printf OUTPUT "Activity (tube entries): " . (($iterations->[2]))  . "\n";
